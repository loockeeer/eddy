...
const command: app.Command = {
  ...
  async run(message) {
    ...
    const embed = new app.MessageEmbed()
      ...
      .setDescription(
        `I'm a dumb discord bot you can train by speaking with me. [Invite me](${invite}) !`
      )
      .addField(
        "Owner",
        (await message.client.users.fetch(process.env.OWNER as string)).tag,
        true
      )
    ...
    if (message.client.uptime) {
      embed.addField(
        "Uptime",
        app.dayjs(Date.now() - message.client.uptime).fromNow(),
        true
      )
    }
