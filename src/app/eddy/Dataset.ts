...
export class Dataset {
    ...
    static createDataset(name: string, ownerID: Discord.Snowflake, ownerKind: TargetKinds): DatasetInterface {
        if(Dataset.exists(name)) throw new DatasetExistsError(`Dataset with name "${name}" already exists`)
        const dataset = {
            name,
    ...
    static deleteDataset(name: string): DatasetInterface {
        if(!Dataset.exists(name)) throw new DatasetNotExistsError(`Dataset with name "${name}" does not exists`)
    ...
    static changeGlobalPermission(name: string, newPermission: Permissions): Permissions {
        if(!Dataset.exists(name)) throw new DatasetNotExistsError(`Dataset with name "${name}" does not exists`)
    ...
    static addSpecificPermission(name: string, target: Discord.Snowflake, targetKind: TargetKinds, permission: Permissions): DatasetPermission {
        if(!Dataset.exists(name)) throw new DatasetNotExistsError(`Dataset with name "${name}" does not exists`)
    ...
    static deleteSpecificPermission(name: string, target: Discord.Snowflake): DatasetPermission {
        if(!Dataset.exists(name)) throw new DatasetNotExistsError(`Dataset with name "${name}" does not exists`)

        const dataset = datasets.ensure(name, {} as DatasetInterface)
        const toDelete = dataset.specificPermissions.find(el => el.target === target)

        if(!toDelete) throw new Error(`No specific permission for user/guild with id ${target}`)

        datasets.set(name, dataset.specificPermissions.filter(el=>el.target!==target), "specificPermissions")
    ...
    static getByOwnerID(ownerID: Discord.Snowflake) {
        ...
        if(!name) return new DatasetNotExistsError(`User/Guild with ID ${ownerID} does not have any dataset`)
        return new Dataset(name)
    }
