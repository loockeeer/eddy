...
export namespace FetchQueue {
  export function add(datasetName: string) {
    if (fetchQueue.has(datasetName))
      throw new Error(`Dataset ${datasetName} fetch already in queue !`)
  ...
  export function pop() {
    ...
    const queue = Array.from(fetchQueue.entries()).sort(
  ...
  export function first() {
    ...
    return Array.from(fetchQueue.entries()).sort(
      (a, b) => a[1].time - b[1].time
    )[0][0]
  }
  ...
  export function setStatus(datasetName: string, status: number) {
    if (!exists(datasetName))
      throw new Error(`Dataset ${datasetName} is not in the queue !`)
