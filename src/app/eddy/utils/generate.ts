...
function generateText(dataset: Dataset, content: string, executor: Discord.Message, write: boolean): string {
    return "Je suis Eddy Malou le savant congolais. Poupidoupidou"
}

export function generate(dataset: Dataset, content: string, executor: Discord.Message): string {
    ...
    for(const specific of dataset.specificPermissions) {
        ...
        if (specific.targetKind === TargetKinds.USER && executor?.id === specific.target) userPermission = specific.permission
    }
    ...
    return generateText(dataset, content, executor, permission === Permissions.WRITE)
}